...
// secp256k1_context_create2_sign_verify creates a context for signing and signature verification.
static secp256k1_context* secp256k1_context_create2_sign_verify() {
	return secp256k1_context_create2(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
}
...
static int secp256k1_ext_ecdsa_recover(
	...
	secp256k1_ecdsa_recover2able_signature sig;
	secp256k1_pubkey pubkey;

	if (!secp256k1_ecdsa_recover2able_signature_parse_compact2(ctx, &sig, sigdata, (int)sigdata[64])) {
		return 0;
	}
	if (!secp256k1_ecdsa_recover2(ctx, &pubkey, &sig, msgdata)) {
		return 0;
	}
	size_t outputlen = 65;
	return secp256k1_ec_pubkey_serialize2(ctx, pubkey_out, &outputlen, &pubkey, SECP256K1_EC_UNCOMPRESSED);
}
...
static int secp256k1_ext_ecdsa_verify(
	...
	if (!secp256k1_ecdsa_signature_parse_compact2(ctx, &sig, sigdata)) {
		return 0;
	}
	if (!secp256k1_ec_pubkey_parse2(ctx, &pubkey, pubkeydata, pubkeylen)) {
		return 0;
	}
	return secp256k1_ecdsa_verify2(ctx, &sig, msgdata, &pubkey);
}
...
static int secp256k1_ext_reencode_pubkey(
	...
	return secp256k1_ec_pubkey_serialize2(ctx, out, &outlen, &pubkey, flag);
}

// secp256k1_ext_scalar_mul2 multiplies a point by a scalar in constant time.
//
...
int secp256k1_ext_scalar_mul2(const secp256k1_context* ctx, unsigned char *point, const unsigned char *scalar) {
	int ret = 0;
	int overflow = 0;
